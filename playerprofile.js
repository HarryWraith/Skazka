...
(function () {
  ...
  async function loadCharacters() {
    ...
    if (!data?.characters?.length) throw new Error('No characters found.');
  ...
  function renderCharTabs(chars, activeId) {
    TABLIST.innerHTML = chars
      .map((c) => {
        ...
        const src = c.icon?.src || c.avatar?.src || c.portrait?.src || '';
        const alt = c.icon?.alt || c.avatar?.alt || c.portrait?.alt || c.name;
        const visual = src
          ? `<img class="char-tab-avatar${isIcon ? ' icon' : ''}" src="${esc(src)}" alt="${esc(alt)}">`
          : `<span class="char-tab-avatar char-tab-initials" aria-hidden="true">${esc(initials(c.name))}</span>`;

        const active = c.id === activeId;
        return `
          <button
            type="button"
            class="char-tab${active ? ' is-active' : ''}"
            id="char-tab-${esc(c.id)}"
            role="tab"
            aria-selected="${active}"
            aria-controls="char-panel"
            tabindex="${active ? '0' : '-1'}"
            data-id="${esc(c.id)}"
          >
            ${visual}
  ...
  function renderMain(c) {
    ...
    // Make the main area the single tabpanel the character tabs control.
    // (All char tabs point to the same panel; content inside changes with selection.)
    main.id = 'char-panel';
    main.setAttribute('role', 'tabpanel');
    main.setAttribute('tabindex', '0'); // focusable for keyboard users
    main.setAttribute('aria-labelledby', `char-tab-${c.id}`);

    // Header with KPIs; mini stat panes directly below (HP/AC/Speed/Init)
    const header = html`
      <header class="page-hero">
        <h2 class="skz-viking">${esc(c.name)}</h2>
        <p class="sub">${esc(c.title || `${c.class} • Level ${c.level}`)}</p>
        <div class="kpi-grid kpi-mini" role="list">
          ${kpi('Level', c.level)} ${kpi('Class', c.class)} ${kpi('Ancestry', c.ancestry)}
    ...
    const tabs = html`
      <nav class="pp-tabs" role="tablist" aria-label="Character Sections">
        <button
    ...
    // Panels
    const bioPanel = html`
      <article id="tab-bio" class="pp-panel is-active" role="tabpanel" aria-labelledby="sec-bio">
        <h2 class="tb-title skz-title">Identity</h2>
        <div class="pp-bio-grid">
          <div>
            <ul class="skz-ol">
              ${c.player ? `<li><strong>Player:</strong>&nbsp; ${esc(c.player)}</li>` : ''}
              ${c.bio?.aliases?.length
                ? `<li><strong>Aliases:</strong>&nbsp; ${esc(c.bio.aliases.join(', '))}</li>`
                : ''}
              ${c.bio?.origin ? `<li><strong>Origin:</strong>&nbsp; ${esc(c.bio.origin)}</li>` : ''}
              ${c.alignment ? `<li><strong>Alignment:</strong>&nbsp; ${esc(c.alignment)}</li>` : ''}
            </ul>

            ${c.bio?.personality || c.bio?.ideals || c.bio?.bonds || c.bio?.flaws
              ? `<h3 class="tb-title skz-title">Personality</h3>`
              : ''}
            ${c.bio?.personality ? `<p>${esc(c.bio.personality)}</p>` : ''}

            <div class="insp-row">
              ${c.bio?.ideals ? `<span class="pill">Ideal: ${esc(c.bio.ideals)}</span>` : ''}
              ${c.bio?.bonds ? `<span class="pill">Bond: ${esc(c.bio.bonds)}</span>` : ''}
              ${c.bio?.flaws ? `<span class="pill">Flaw: ${esc(c.bio.flaws)}</span>` : ''}
            </div>
          </div>
        </div>
      </article>
    ...
    const backstoryPanel = html`
      <article id="tab-backstory" class="pp-panel" role="tabpanel" aria-labelledby="sec-backstory">
        ${c.backstory
          ? `<h2 class="tb-title skz-title">Before the Journey</h2><p>${esc(c.backstory)}</p>`
          : `<p class="muted">No backstory yet.</p>`}
      </article>
    `;

    const chroniclePanel = html`
      <article id="tab-chronicle" class="pp-panel" role="tabpanel" aria-labelledby="sec-chronicle">
                ...
                return `
                  <div class="event">
                    <button class="event-header" type="button" aria-expanded="false">
                      ...
                      ${
                        iconSrc
                          ? `<img class="rune" src="${esc(iconSrc)}" alt="${esc(iconAlt)}">`
                          : ''
                      }
                    </button>
    ...
    const inventoryPanel = html`
      <article id="tab-inventory" class="pp-panel" role="tabpanel" aria-labelledby="sec-inventory">
        <h2 class="tb-title skz-title">Inventory</h2>
        <ul class="skz-ol">
          ${c.inventory?.items?.length
            ? `<li><strong>Items:</strong> ${esc(c.inventory.items.join(', '))}</li>`
            : ''}
          ${c.inventory?.spellbook?.length
            ? `<li><strong>Spellbook:</strong> ${esc(c.inventory.spellbook.join(', '))}</li>`
            : ''}
        </ul>
      </article>
    `;
  ...
  function renderSide(c) {
    ...
    side.innerHTML = `
      <article class="tb-panel">
        ${
          c.portrait?.src
            ? `<img src="${esc(c.portrait.src)}" alt="${esc(c.portrait.alt || c.name)}">`
            : ''
        }
      </article>
    `;
  ...
  function wireCharSwitch(chars) {
    const tabs = TABLIST.querySelectorAll('.char-tab');
    tabs.forEach((btn) => {
      ...
      btn.addEventListener('keydown', (e) => {
        ...
        const list = Array.from(TABLIST.querySelectorAll('.char-tab'));
        ...
        list[n].focus();
        // Do not auto-activate on arrow (roving focus only). Activate on Enter/Space.
      });
      btn.addEventListener('keydown', (e) => {
  ...
  // Bootstrap
  (async function init() {
    // Keep the skeleton valid: do NOT claim tablist until tabs exist.
    // Also provide polite loading status for AT.
    TABLIST.setAttribute('aria-label', TABLIST.getAttribute('aria-label') || 'Characters');
    TABLIST.setAttribute('aria-busy', 'true');
    if (TABLIST.getAttribute('role') === 'tablist') TABLIST.removeAttribute('role');
    const skel = TABLIST.querySelector('.skeleton-tabs');
    if (skel) {
    ...
    try {
      ...
      // Now we have real tabs; claim the tablist role.
      ...
      TABLIST.innerHTML = `<div class="error" role="alert">Failed to load characters.</div>`;
    }
  })();
})();
