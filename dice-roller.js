...
(function () {
     ...
     function getAbilityModsFromLeft() {
          try {
               const saved = JSON.parse(
                    localStorage.getItem("skzAttrV3") || "null"
               );
               if (
                    !saved ||
                    !Array.isArray(saved.rolls) ||
     ...
     // Parse "NdMkh2+3", "4d6dl1-1", "3d8+DEX" etc.
     function parseNotation(str) {
          const s = (str || "").trim().toUpperCase();
          const m = s.match(
               /^(\d*)D(\d+)(?:(K[HL]|D[HL])(\d+))?\s*([+\-]\s*(\d+|STR|DEX|CON|INT|WIS|CHA))?$/i
          );
          if (!m) return null;
          ...
          if (op && x) {
               ...
               const sortedIdx = all
                    .map((v, i) => ({ v, i }))
                    .sort((a, b) => a.v - b.v); // asc
               if (op === "kh") {
                    const keep = new Set(
                         sortedIdx.slice(-count).map((o) => o.i)
                    );
                    kept = all.filter((_, i) => keep.has(i));
                    dropped = all.filter((_, i) => !keep.has(i));
               } else if (op === "kl") {
                    const keep = new Set(
                         sortedIdx.slice(0, count).map((o) => o.i)
                    );
                    kept = all.filter((_, i) => keep.has(i));
                    dropped = all.filter((_, i) => !keep.has(i));
               } else if (op === "dh") {
                    ...
                    kept = all.filter((_, i) => !drop.has(i));
                    dropped = all.filter((_, i) => drop.has(i));
               } else if (op === "dl") {
                    const drop = new Set(
                         sortedIdx.slice(0, count).map((o) => o.i)
                    );
     ...
     function macrosSave(list) {
          try {
               localStorage.setItem(LS_MACROS, JSON.stringify(list));
          } catch {}
     }
     function initMacros() {
          const listEl = $("#skz-macro-list");
          if (!listEl) return;
          const render = () => {
               listEl.innerHTML = "";
          ...
          $("#skz-macro-add")?.addEventListener("click", () => {
               ...
               const arr = macrosLoad();
               arr.push({ name, notation });
               macrosSave(arr);
               $("#skz-macro-name").value = "";
               $("#skz-macro-notation").value = "";
               render();
          });
     }
     ...
     function bindExport() {
          ...
          $("#skz-export-csv")?.addEventListener("click", () => {
               const rows = historyLoad().map(
                    (h) => `"${h.ts}","${(h.text || "").replace(/"/g, '""')}"`
               );
          ...
          $("#skz-export-json")?.addEventListener("click", () => {
               navigator.clipboard?.writeText(
                    JSON.stringify(historyLoad(), null, 2)
               );
               announce("History copied as JSON");
          });
          $("#skz-export-print")?.addEventListener("click", () => {
               const w = window.open("", "_blank");
               const html = `<pre style="font:14px/1.4 ui-monospace,Menlo,Consolas,monospace;padding:16px">${historyLoad()
                    .map((h) => `${h.ts}  ${h.text}`)
                    .join("\n")}</pre>`;
               if (w) {
     ...
     function initCenter() {
          ...
          const updateMode = () => {
               ...
               modeSlider.setAttribute(
                    "aria-valuenow",
                    String(modeSlider.value)
               );
          ...
          const tips = {
               4: "Damage dice (daggers)",
               6: "Common damage / heals",
               8: "Martial damage",
               10: "Percentile tens",
               12: "Greataxe etc.",
               20: "Checks & attacks",
          ...
          root.querySelectorAll(".skz-die").forEach((btn) => {
               ...
               btn.addEventListener("contextmenu", (e) => {
                    ...
                    updateBadge();
                    setTitle();
                    if (typeof updateComposition === "function")
                         updateComposition(); // safe if you added composition preview
               });
          });
          ...
          // Help tip (once)
          {
               ...
               if (grid && !root.querySelector("#skz-click-tip")) {
                    ...
                    tip.textContent =
                         "Tip: Left-click adds to the dice count. Right-click sets it to 1. Click Roll to roll all selected dice.";
          ...
          if (!rollBar) {
               ...
               (
                    tip || root.querySelector(".skz-die-grid")
               ).insertAdjacentElement("afterend", rollBar);
          }
          ...
          function renderLastCombo({
               ...
               const panel = lastBody?.parentElement;
               if (!lastBody) return;
               panel?.classList.remove("skz-crit", "skz-fumble");
               panel?.classList.add("skz-pulse");
               lastBody.innerHTML = `${breakdown.join(
                    "<br>"
               )}<br><em>${mode}</em> ${fmtSigned(
                    mod
               )} → <strong>${totalText}</strong>`;
               appendHistoryEntry(
                    `${breakdown.join(" | ")} ${mode} ${fmtSigned(
                         mod
                    )} = ${totalText}`,
                    totalValue
               );
          ...
          function rollSelected() {
               ...
               if (mode === "normal") {
                    ...
                    selected.forEach(({ sides, n }) => {
                         ...
                         const pairsStr = pairs
                              ...
                              .join(" ");
                         parts.push(
                              `${n}d${sides} (${mode}): ${pairsStr} → keep [${kept.join(
                                   ", "
                              )}] = ${subtotal}`
                         );
                    });
               ...
               renderLastCombo({
                    ...
                    mod,
                    breakdown: parts,
                    totalText: `${grandTotal} ${fmtSigned(mod)} = ${final}`,
                    totalValue: final,
               });
          }
          ...
          $("#skz-copy-last")?.addEventListener("click", () => {
               ...
               navigator.clipboard?.writeText(text).then(() => {
                    ...
                    setTimeout(
                         () => ($("#skz-copy-last").textContent = "Copy"),
                         900
                    );
               });
          });
      ...
      <div class="skz-notation">
        <input id="skz-notation" class="skz-input" placeholder="Dice notation e.g. 8d6+3 or 2d20kh2+2">
    ...
    `;
          ...
          const reflectSound = () =>
               (soundBtn.textContent =
                    soundBtn.dataset.on === "1" ? "🔊 Sound" : "🔇 Sound");
          reflectSound();
          soundBtn.addEventListener("click", () => {
               soundBtn.dataset.on = soundBtn.dataset.on === "1" ? "0" : "1";
               localStorage.setItem(
                    SOUND_LS,
                    soundBtn.dataset.on === "1" ? "1" : "0"
               );
               reflectSound();
          });
          ...
          $("#skz-notation-roll")?.addEventListener("click", () => {
               ...
               const lastBody = $("#skz-last-body");
               const label = `${notation.toUpperCase()} ${
                    parsed.modSrc
                         ? `( ${parsed.modSrc} ${fmtSigned(parsed.mod)} )`
                         : parsed.mod
                         ? fmtSigned(parsed.mod)
                         : ""
               }`.trim();
               if (count === 1) {
                    lastBody.innerHTML = `${label}: rolls ${JSON.stringify(
                         parsed.all
                    )} ${
                         parsed.dropped.length
                              ? `drop ${JSON.stringify(
                                     parsed.dropped
                                )} → keep ${JSON.stringify(parsed.kept)} `
                              : ""
                    ...
                    lastBody.innerHTML = `${label} ×${count}: min <strong>${min}</strong> • avg <strong>${avg}</strong> • max <strong>${max}</strong> ${sparkline(
                         totals
                    )}`;
                    appendHistoryEntry(
                         `${label} ×${count}: [${totals.join(", ")}]`,
                         null
                    );
               }
               playTick();
    ...
    `;
          ...
          document.addEventListener("keydown", (e) => {
               ...
               if (dieMap[key]) {
                    root.querySelector(
                         `.skz-die[data-die="${dieMap[key]}"]`
                    )?.click(); // increments planned count
               } else if (key === "r") {
     ...
     function attrSave() {
          try {
               localStorage.setItem(LS_ATTR, JSON.stringify(attrState));
          } catch {}
     }
     function attrLoad() {
          try {
               return JSON.parse(localStorage.getItem(LS_ATTR) || "null");
          } catch {
               return null;
          }
     }
     ...
     function renderAttr() {
          ...
          if (!Array.isArray(attrState.assignment))
               attrState.assignment = [null, null, null, null, null, null];
      ...
      `;
               $("#skz-attr-sort").addEventListener(
                    "click",
                    sortDescIntoUnlocked
               );
               $("#skz-attr-std").addEventListener("click", useStandardArray);
               $("#skz-attr-export-mods").addEventListener(
                    "click",
                    copyWithMods
               );
          }
          ...
          ATTRS.forEach((name, rowIdx) => {
               ...
               lockBtn.className =
                    "skz-lock" + (attrState.locks[rowIdx] ? " is-locked" : "");
               ...
               if (idx == null) {
                    ...
                    ph.className = "skz-placeholder";
                    ph.textContent = "Drop score here";
                    ...
                    card.dataset.scoreId = String(sc.id);
                    card.setAttribute(
                         "aria-label",
                         `Score ${sc.total} for ${name}, drag to swap`
                    );
                    card.innerHTML = `<span class="skz-card-total"><strong>${
                         sc.total
                    }</strong></span>
                          <span class="skz-card-chips">${chipsHTML(sc)}</span>
                          <button class="skz-rebtn" title="Reroll this score" data-re="${
                               sc.id
                          }">⟲</button>`;
          ...
          pool.addEventListener("drop", (e) => {
               ...
               if (fromAttr !== "") {
                    ...
                    if (idx != null) {
                         ...
                         if (!attrState.pool.includes(idx))
                              attrState.pool.push(idx);
                         attrSave();
                         renderAttr();
                    }
               }
     ...
     function badgeChips(sc) {
          ...
          return `<span class="skz-badge">${sc.kept.join("+")}${
               sc.dropped != null ? ` (drop ${sc.dropped})` : ""
          }</span>`;
     }
     function chipsHTML(sc) {
          ...
          return (
               sc.kept
                    .map((v) => `<span class="skz-chip">${v}</span>`)
                    .join("") +
               `<span class="skz-chip skz-chip--dropped">${sc.dropped}</span>`
          );
     }
     ...
     function bindAttrDnD() {
          ...
          cards.forEach((card) => {
               ...
               card.addEventListener("dragend", () => {
                    ...
                    document
                         .querySelectorAll(".skz-attr-cell.dragover")
               ...
               card.addEventListener("click", () => {
                    ...
                    document
                         .querySelectorAll(".skz-card.selected")
                         .forEach((el) => {
                              if (Number(el.dataset.attrIndex) !== from)
          ...
          cells.forEach((cell) => {
               ...
               cell.addEventListener("drop", (e) => {
                    ...
                    if (fromAttrData !== "") {
                         ...
                         attrState.pool = attrState.pool.filter(
                              (i) => i !== scoreIdx
                         );
     ...
     function swapAssignments(a, b) {
          const A = attrState.assignment[a],
               B = attrState.assignment[b];
     ...
     function sortDescIntoUnlocked() {
          ...
          const sorted = allIdx.sort(
               (i, j) => attrState.rolls[j].total - attrState.rolls[i].total
          );
          const result = [...attrState.assignment];
          const lockedSet = new Set();
          attrState.locks.forEach((v, i) => {
               if (v && result[i] != null) lockedSet.add(result[i]);
          });
     ...
     document.addEventListener("dragstart", (e) => {
          ...
          if (el.dataset.poolScore != null) {
               e.dataTransfer.setData("fromPool", el.dataset.poolScore);
          }
     });
})();
