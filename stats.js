...
function buildSessionSearchIndex(ctx) {
  ...
  for (const s of sessions) {
    ...
    if (s.inspiration?.awarded) {
      parts.push(s.inspiration.awarded.map((a) => a.reason || "").join(" "));
    }
    if (s.inspiration?.used) {
      parts.push(s.inspiration.used.map((u) => u.reason || "").join(" "));
    }
...
function mdToHtml(md) {
  ...
  t = t.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (m, txt, href) => {
    ...
    return `<a href="${url}" target="_blank" rel="noopener">${esc(txt)}</a>`;
  });
  ...
  const blockStart = /^(<h[1-3]>|<figure\b|<ul\b|<ol\b|<blockquote\b|<pre\b)/;
  t = t
    .split(/\n{2,}/)
...
function aggregate(data) {
  ...
  for (const s of sessions) {
    ...
    for (const [id, st] of Object.entries(cs)) {
      if (!agg.perChar[id]) {
        // allow guests
        agg.perChar[id] = JSON.parse(JSON.stringify(agg.perChar[chars[0]?.id]));
        agg.perChar[id].id = id;
        agg.perChar[id].name = byId[id]?.name || id;
        for (const k of Object.keys(agg.perChar[id]))
          if (typeof agg.perChar[id][k] === "number") agg.perChar[id][k] = 0;
      }
      ...
      // inspiration bank
      ctot.bank += (st.inspiration_awarded || 0) - (st.inspiration_used || 0);
...
function renderKpis(mount, ctx) {
  ...
  const meleeRate = meleeAtt ? Math.round((agg.meleeHits / meleeAtt) * 100) : 0;
  const spellRate = spellAtt ? Math.round((agg.spellHits / spellAtt) * 100) : 0;
...
function renderLeaderboards(mount, ctx) {
  ...
  function top5(
    ...
    const rows = arr
      .filter(filter)
      .map((c) => ({ name: byId[c.id]?.name || c.id, val: getVal(c) || 0 }))
      .sort((a, b) => b.val - a.val)
      .slice(0, pad);

    const html = rows.map(
      (r) =>
        `<tr><td>${esc(r.name)}</td><td class="num">${fmt(r.val)}</td></tr>`
    );

    while (html.length < pad) {
      html.push(`<tr class="empty"><td>—</td><td class="num">—</td></tr>`);
    }
    return html.join("");
  }
  ...
  const sections = [
    {
      ...
      opts: {
        filter: (c) => c.melee_hits + c.melee_misses >= MIN_ATTEMPTS,
        fmt: (v) => `${fmtInt(v)}%`,
      },
    },
    {
      ...
      opts: {
        filter: (c) => c.spell_hits + c.spell_misses >= MIN_ATTEMPTS,
        fmt: (v) => `${fmtInt(v)}%`,
      },
    },
        ...
        filtered
          .map((s) => {
            ...
            const tags = (s.arc_tags || [])
              .map((t) => `<span class="tag">${hl(t, query)}</span>`)
              .join(" ");
            return `
...
function renderSessionDetail(centerMount, s, ctx) {
  ...
  // KPIs (session)
  const pcDamage =
    sum("total_melee_dmg") + sum("total_spell_dmg") + sum("total_aoe_dmg");
  ...
  const biggestHealStr = biggestHeal.amount
    ? `${esc(byId[biggestHeal.actor]?.name || biggestHeal.actor)} — ${fmtInt(
        biggestHeal.amount
      )} HP`
    : "—";
  ...
  const inspAwarded =
    (insp.awarded || [])
      .map(
        (a) =>
          `<div><strong>${esc(byId[a.to]?.name || a.to)}</strong> — ${esc(
            a.reason || ""
          )}</div>`
      )
  ...
  const inspUsed =
    (insp.used || [])
      .map(
        (u) =>
          `<div><strong>${esc(byId[u.by]?.name || u.by)}</strong> — ${esc(
            u.reason || ""
          )}</div>`
      )
      .join("") || `<div class="muted">None</div>`;
